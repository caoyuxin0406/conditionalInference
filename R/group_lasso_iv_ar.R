...
#' @param ridge_term L2 regularization term (if applicable)
...
#' @param C0 Threshold of pre-test on instrument strength
#'
...
group_lasso_iv_ar <- function(Y,
  ...
  # grplasso(x=rep(1,p),y=np.zeros(p), weights=rep(penalty,length(y)), lamda=1.)
...
#'
#' @param binst A bigaussian instance object
...
#' @param level Significance level of F-test
#'
#' @return The p-value and confidence interval of AR test statistic.
#' @export
#'
#' @importFrom stats pf
#' @importFrom stats qf
...
#' naive_inference_ar(binst=b,
#'                    pass_pre_test=TRUE,
#'                    compute_intervals=TRUE)
#'
#' # Naive inference for AR test statistic without confidence intervals
#' # given passing the pre-test
#' naive_inference_ar(binst=b, pass_pre_test=TRUE)
#'
#' # Naive inference for AR test statistic given not passing the pre-test
#' naive_inference_ar(binst=b, pass_pre_test=FALSE)
naive_inference_ar <- function(binst,
                               pass_pre_test,
                               parameter=NULL,
                               Sigma_11=1,
                               Sigma_12=0.8,
  ...
  return_list <- list("pval" = pval, "interval" = interval)
  return(return_list)
}
